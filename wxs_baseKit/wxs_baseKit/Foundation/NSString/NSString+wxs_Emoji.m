...
#import "NSString+wxs_Emoji.h"

@implementation NSString (wxs_Emoji)

static NSDictionary * wxs_s_unicodeToCheatCodes = nil;
static NSDictionary * wxs_s_cheatCodesToUnicode = nil;

+ (void)wxs_initializeEmojiCheatCodes
{
    ...
    @synchronized(self) {
        wxs_s_unicodeToCheatCodes = forwardMap;
        wxs_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)wxs_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!wxs_s_cheatCodesToUnicode) {
        [NSString wxs_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [wxs_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)wxs_stringByReplacingEmojiUnicodeWithCheatCodes
{
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [wxs_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
...
- (BOOL)wxs_isEmoji
{
    return [self wxs_isEmojiWithUnicode] || [self wxs_isEmojiWithCheatCodes];
}
...
- (NSString *)wxs_removedEmojiString {
    ...
    [self enumerateSubstringsInRange:NSMakeRange(0, [self length])
                             options:NSStringEnumerationByComposedCharacterSequences
                          usingBlock: ^(NSString* substring, NSRange substringRange, NSRange enclosingRange, BOOL* stop) {
                              [buffer appendString:([substring wxs_isEmoji])? @"": substring];
                          }];
